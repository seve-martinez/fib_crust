        ...
        Some(data) => {
            match data.downcast::<PyList>() {
                Ok(raw_data) => {
                    ...
                    let fib_numbers: Vec<u64> = processed_results.iter()
                        .map(|x| fibonacci_number(*x))
                        .collect();
                    config.set_item("NUMBER RESULT", fib_numbers)?;
                },
                Err(_) => Err(PyTypeError::new_err("param is not a list of integers")).unwrap()
            }
        },
        None => println!("param is not in the config")
    }
